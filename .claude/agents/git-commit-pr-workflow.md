...
4. **Run the full test suite — mandatory, not conditional on being reminded.** Run `./mvnw test` (or `./mvnw clean test` if you have reason to distrust stale build output). This project's test suite includes a Testcontainers-based integration test that needs Docker running: check with `docker info`; if it's not up, start Docker Desktop (`Start-Process "C:\Program Files\Docker\Docker\Docker Desktop.exe"` via PowerShell, or the bash equivalent) and poll until ready before running tests. **If the suite fails, stop here and report the failure — do not open a PR.**
   - **If this commit touches anything under `frontend/`**, its test suite is equally mandatory. Node LTS + npm are installed on this machine (see `README.md`'s Prerequisites) — from `frontend/`, run `npm ci && npm run typecheck && npm test && npm run lint && npm run build`. Treat a failure here exactly like an `./mvnw test` failure: stop, do not open a PR. If Node/npm are ever missing in a given environment, that's a "never install new software" stop-and-report situation (note 4 above), not a green light to skip the suite and merge anyway.
...
6. **Self-review**: invoke the `/code-review` skill against the diff (correctness bugs plus reuse/simplification/efficiency cleanups) instead of eyeballing it yourself. You have no Write/Edit access, so don't pass `--fix` — findings only. If a finding is genuinely fixable with just Bash (e.g. stripping a leftover debug line), fix it in a follow-up commit and push again before continuing. If a finding needs a real code change, stop and report it (to the invoking session, for `java-springboot-expert`/`react-frontend-expert` to address) rather than merging known-bad code, even in `full-auto` mode.
