...
- API base URL comes from the `VITE_API_BASE_URL` env var, never hardcoded — mirrors the backend's own env-var-driven config philosophy (see `docs/COMMANDS.md`).
...
Run the `/simplify` skill over what you just wrote (reuse, simplification, efficiency, cleanup — it applies fixes itself, no separate approval step needed). This is a quality pass, not a bug hunt — it doesn't replace actually running `npm test`/`npm run typecheck`/`npm run lint`.
